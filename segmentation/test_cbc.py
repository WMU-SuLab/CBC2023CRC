...
def modelEvalution(i, net, savePath, use_cuda=False, dataset='out', input_ch=3, config=None):
    ...
    for k in t:
        ...
        #cv2.imwrite(os.path.join(data_path, f"{image_basename[k].split('.')[0]}_0.6.{image_basename[k].split('.')[-1]}"), (PredAll[0, 0, :, :] > 0.7) * 255)
        # cv2.imwrite(os.path.join(data_path, f"{image_basename[k].split('.')[0]}_0.7.{image_basename[k].split('.')[-1]}"), (PredAll[0, 0, :, :] > 0.7) * 255)
        #
        # cv2.imwrite(os.path.join(data_path, f"{image_basename[k].split('.')[0]}_0.8.{image_basename[k].split('.')[-1]}"), (PredAll[0, 0, :, :] > 0.8) * 255)
        # cv2.imwrite(os.path.join(data_path, f"{image_basename[k].split('.')[0]}_0.9.{image_basename[k].split('.')[-1]}"), (PredAll[0, 0, :, :] > 0.9) * 255)
