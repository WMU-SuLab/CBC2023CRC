...
def main():
    ...
    image_path ='../CBC-GAN/data/CBC_ft'
    ...
    train_dataset = ImageFolderCustom(os.path.join(image_path, "training"),['label11','label00','images','label'],
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset,
                                               ...
                                               num_workers=nw)
    # ¼ÓÔØÑéÖ¤Êý¾Ý¼¯
    validate_dataset =ImageFolderCustom(os.path.join(image_path, "test"),['label11','label00','images','label'],
    ...
    for epoch in range(epochs):
        ...
        if val_loss < val_best_loss:
            torch.save(model.state_dict(), r"./weight/val_best_model_v2.pth")
            val_best_loss = val_loss

        if train_loss < train_best_loss:
            torch.save(model.state_dict(), r"./weight/train_best_model_v2.pth")
            train_best_loss = train_loss

        if val_acc > best_acc:  # acc improve save weight
            best_acc = val_acc
            torch.save(model.state_dict(), r"./weight/train_best_acc_model_v2.pth")
